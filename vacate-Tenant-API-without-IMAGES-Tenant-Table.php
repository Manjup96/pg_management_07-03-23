...
$get_tenant_Details_sql_query="SELECT `tenant_name`, `tenant_email`, `tenant_mobile`,  `tenant_aadhar_number`, `joining_date` FROM `tenant_registration_without_image` WHERE tenant_mobile='$tenant_mobile_no'";
...
if ($result->num_rows > 0)
{
  ...
  $tbl_vacated_tenant_sql = "INSERT INTO `tbl_vacated_tenant`(`id`, `tenant_name`, `tenant_email`, `tenant_mobile`, `floor_no`, `room_no`, `bed_no`,`tenant_address`,`tenant_aadhar_number`,`tenant_image`,`tenant_documents`,`joined_date`) values('$id','$tenant_name','$tenant_email','$tenant_mobile','$floor_no', '$room_no', '$bed_no','$tenant_address','$tenant_aadhar_number','$tenant_image','$tenant_documents','$joining_date')";
...
// {
//     $sql="DELETE FROM `tenant_registration_without_image` WHERE `tenant_mobile`=$tenant_mobile_no";
