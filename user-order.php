...
<title>Suhas Diagnostic Centre | Orders</title>
...
<meta name="description" content="Suhas is most Sophisticated Pathology,Radiology,Cardiology lab and diagnostics center known for excellent customer satisfaction and preventive healthcare in Bangalore.">
  <meta name="keywords" content="Pathology lab,Radiology,x-ray,urine test,blood test,diagnostic centre,covid test,sugar test , choesterol test ,kidney test,scanning,home collection,Diagnostic Centres in south bangalore,Top diagnostic Labs in south bangalore,top diagnostics labs in bangalore,top pathology lab in bangalore,Bangalore diagnostics centres,Suhas Diagnostics Centre,Top Laboratories in bangalore,Diagnostics Centres near me,Diagnostic Centres near hanmant nagar">
...
s1.async=true;
s1.src='https://embed.tawk.to/60fe4ea5649e0a0a5ccdee07/1fbgm6ee0';
s1.charset='UTF-8';
  ...
  <div class="logo"><a href="#"><img src="img/suhas_logo.png" alt="suhas-logo" class="img-fluid lazyload suhas-logo"></a></div>
    ...
    <div class="navbar-collapse collapse width" id="navbarSupportedContent">
      <ul class="navbar-nav mr-auto first-navbar">
        ...
        <a href="checkout.php" class="basket-cart">
        ...
        <!--<li class="nav-item sub-menu-item">
          <?php echo "Welcome".$_SESSION["name"];?>&nbsp;<a href="Logout.php"><i class="fa fa-user"></i>Logout</a>
        </li>-->
          <?php }else
          {
          ?>
...
<style type="text/css">
  ...
  #user-h{text-align-last: end;}
</style>

<section>
  <div class="container">
    <div class="col-md-12">
<div id="user-h"><h6>WELCOME <?php echo $_SESSION["name"];?></h6></div><br/>

<div class="tab">
  <a  class="tablinks" href="user-report.php">Reports</a>
  <a class="tablinks" href="user-order.php">Orders</a>
  <a class="tablinks" href="profile.php">Profile</a>
  <a class="tablinks" href="Logout.php">Logout</a>
</div>
...
$phone=$_SESSION["phone"];
  $sql="select * from orders where p_phone = '$phone'";
  $result=$conn->query($sql);
?>
  <div class="table-responsive" id="show_table">
    <table class="table  table-bordered table-hover" id="table">
      ...
      ?>
          <tr>
            ...
            <td><?php echo $row["scheduled_date"] ?> &nbsp; <?php echo $row["scheduled_time"] ?></td>
            <td><?php echo $row["total"] ?></td>
           <?php  $date_now = new DateTime();
           $sd=($row["scheduled_date"]);
            $sdate=new DateTime("$sd");
               // $cdate=strtotime("$date_now");
            if($date_now > $sdate){ ?>
             <td><?php echo "CLOSED" ?></td> <?php
          }
            elseif($row["Actions"] == "Pending") {?>
            <td><?php echo "Upcoming"; ?></td>
          <?php }
          
          else {?>
            <td><?php echo $row["Actions"] ?></td><?php }
            ?>
            <?php  if($row["Actions"] == "Processed" ) {?>
              <script>
            ...
            <td><input class="btn btn-danger btn-sm " type="button" name="cancel" id="cancel" value="Cancel" onclick="deletefun();"/> 
              <input class="btn btn-success btn-sm" type="button" value="Reshedule" id="reshedule" onclick="reshedulefun();" />
            <?php } ?>
            </td>
          </tr>
        <?php
    ...
    {
      ?>
        <tr><td colspan="7"><? echo "No orders"?></td></tr>
  <?php }
         ?>
      </tbody>
     </table>
          ...
          for(var i=1; i < table.rows.length; i++)
          {
            table.rows[i].cells[6].onclick=function()
            {
              ...
              if(c == true)
              {
                ...
                window.location.replace("cancelorder.php?id="+id);
                ...
                function reshedulefun()
                {
                    ...
                    for(var i=1; i < table.rows.length; i++)
                    {
                        table.rows[i].cells[6].onclick=function()
                        {
                            ...
                            //var email=table.rows[index].cells[3].innerHTML;
                            window.location.assign("reschedule.php?id="+id);
                        };
                    }
                }
</script>   
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="row right-side-blog-page login-modal-content">
                <div class="col-sm-12">
                  <span class="close" type="button" id="btnClosePopup" data-dismiss ="modal" style="float: right;">×</span>
                    <div class="">
...
<script type="text/javascript">
 // var modal = document.getElementById("uploadPresModal");
...
$resumeFileType = pathinfo($dist1,PATHINFO_EXTENSION);
   ...
   // $errorMsg=  "error : Number should be ten digits.";
    ...
    if ($conn->query($sql) === TRUE) 
    {
      ...
      .'<a href="http://suhasdiagnostics.com">suhasdiagnostics.com</a>' . '<br/>' . 
      '<img src="suhasdiagnostics.com/img/suhas_logo.png"  width="160px" height="60px"/>'. '<br/>';
